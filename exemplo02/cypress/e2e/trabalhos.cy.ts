...
describe("Testes sobre o endpoint /trabalhos", () => {
  before(() => {
    ...
    cy.fixture("trabalho_com_autor_com _nome_invalido").as(
      "trabalhoComAutorComNomeInvalido",
    );
    cy.fixture("trabalho_com_autor_com_cfp_invalido").as(
      "trabalhoComAutorComCfpInvalido",
    );
  ...
  test("não deve salvar um trabalho com menos de dois autores", function () {
  ...
  test("não deve salvar um trabalho com uma área inválida", function () {
    ...
    cy.request(requestOptions).then((res) => {
      ...
      const { mensagensDeErro } = res.body;
      expect(mensagensDeErro[0]).to.equal(
        "O trabalho deve ser uma entre as opções: CAE, CET, CBS, CHCSA e MDIS.",
      );
    });
  });
   ...
   test("não deve salvar um trabalho com um autor com nome inválido", function () {
   ...
   test("não deve salvar um trabalho com um autor com gênero inválido", function () {
   ...
   test("não deve salvar um trabalho com um autor com CPF inválido", function () {
     requestOptions.body = this.trabalhoComAutorComCfpInvalido;
